...
const UserGiacenzeView = () => {
  ...
  const loadUserGiacenze = async () => {
    ...
    try {
      ...
      console.log('🔍 Caricando giacenze utente:', selectedUserForGiacenze);
      console.log('🔍 Con filtri:', filters);
      
      const data = await apiCall(`/admin/giacenze?${queryParams}`, {}, token);
  ...
  // Ricarica giacenze quando cambia l'utente selezionato o i filtri
  useEffect(() => {
    loadUserGiacenze();
  }, [selectedUserForGiacenze, filters]);

  // Applica filtri locali (per filtri che non possono essere fatti lato server)
  useEffect(() => {
    let filtered = [...userGiacenze];

    // I filtri principali sono già applicati lato server
    // Qui puoi aggiungere filtri aggiuntivi se necessario
